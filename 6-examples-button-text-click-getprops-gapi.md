...
In below example the page / component will be loaded with fetched json by internal GAPI and set the value to `props.btntext` and after user clicks on the button the page / component will re-render and the button's text will change to `Clicled!`.
```tsx
...
const Page: PageEl = (
  ...
  getProps(async (isFront)=>{
    let json = await API["my/internal/api"](null);
